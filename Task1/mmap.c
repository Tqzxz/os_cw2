...
{
	unsigned long newbrk, oldbrk, origbrk, delta;
...
success_unlocked:
	...
	if (mm->brk != origbrk){
		delta = (mm->brk > origbrk) ? (mm->brk - origbrk) : (origbrk - mm->brk);
		atomic_long_inc(&current->brk_count);
		atomic_long_add(delta,&current->brk_bytes);
	}
			      ...
			      unsigned long fd, unsigned long pgoff)
{
	...
	if (!IS_ERR_VALUE(retval)){
		...
		atomic_long_add(orig_len,&current->mmap_bytes);
	}
